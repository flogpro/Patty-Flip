    ...
    <div className="grid grid-cols-3 items-center gap-2 mb-2">
      <div className="flex justify-center min-w-0">
        {phase === 'guessing' ? (
          ...
          <div className="overflow-hidden rounded-lg shrink-0 flex items-center min-w-0" style={{ maxHeight: 50 }}>
            <img
      ...
      <div className="text-center min-w-0 py-0.5 px-2 rounded-md bg-gradient-to-r from-emerald-950/30 to-transparent border-l-2 border-emerald-500/40">
        <p className="text-sm font-bold text-violet-100">Score: <span className="text-emerald-200">{run.totalScore}</span></p>
        <p className="text-[11px] text-violet-200/70">Burgers: {run.burgersRemaining} / {burgersPerRun}</p>
      </div>
        ...
        <div className="flex rounded-md border border-violet-600/50 border-emerald-500/20 overflow-hidden bg-violet-950/40">
          <button
            ...
            className={`px-1.5 py-0.5 text-[10px] font-medium transition focus:outline-none focus:ring-1 focus:ring-violet-400 focus:ring-inset ${
              !suspensefulFlip ? 'bg-gradient-to-r from-emerald-600 to-violet-500 text-white' : 'text-violet-200/80 hover:text-violet-200 hover:bg-violet-900/50'
            }`}
          >
            Fast
          </button>
